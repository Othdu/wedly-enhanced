...
class VideographerBookingScreen extends StatefulWidget {
  final ServiceModel service;

  const VideographerBookingScreen({super.key, required this.service});

  @override
  State<VideographerBookingScreen> createState() =>
      _VideographerBookingScreenState();
}

class _VideographerBookingScreenState extends State<VideographerBookingScreen> {
  ...
  // Selected additions (only interactive section)
  ...
  // Static package data for videographer
  static const Map<String, Map<String, dynamic>> _packages = {
    'details': {
      'title': 'تفاصيل الباقة',
      'items': ['نوع الخدمة: تصوير فيديو احترافي لحفل الزفاف'],
    },
    'types': {
      'title': 'الأنواع المتاحة',
      'items': [
        'هايلايت (Highlight Clip)',
        'فيديو كامل للحفل (Full Coverage)',
        'جلسة تصوير قبل الزفاف (Pre-Wedding Shoot)',
      ],
    },
    'equipment': {
      'title': 'الخامات والمعدات',
      'items': [
        'كاميرا 4K احترافية',
        'عدسات سينمائية',
        'درون (Drone) لتصوير جوي',
        'إضاءة متنقلة + مايك احترافي للصوت',
      ],
    },
    'price': {
      'title': 'السعر',
      'items': [
        'السعر يبدأ من: 4,500 جنيه',
        'يشمل: تصوير + مونتاج + تسليم النسخة النهائية',
      ],
    },
    'additions': {
      'title': 'الإضافات',
      'items': [
        'تصوير درون (جوي)',
        'لقطة بطيئة (Slow Motion Shots)',
        'جلسة خاصة قبل او بعد الفرح',
        'تعديل لوني احترافي (Color Grading)',
      ],
    },
    'timing': {
      'title': 'الوقت والتجهيز',
      'items': [
        'مدة التغطية: من 3 إلى 6 ساعات حسب الحفل',
        'مدة التسليم: من 7 إلى 10 أيام بعد الحدث',
      ],
    },
  };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          // App Bar with videographer image
          ...
          SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                ...
                // Videographer name badge
                _buildVideographerBadge(),
  ...
  Widget _buildVideographerBadge() {
    return Container(
  ...
  Widget _buildPackagesSection() {
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        _buildPackageCard('types', isInteractive: false),
        const SizedBox(height: 16),
        _buildPackageCard('equipment', isInteractive: false),
        const SizedBox(height: 16),
  ...
  Widget _buildPriceSummary() {
    ...
    for (final addition in _selectedAdditions) {
      // Extract the addition text from the key (format: "additions-0", "additions-1", etc.)
        ...
        children: [
          ...
          if (_selectedAdditions.isNotEmpty) ...[
            ...
            ..._selectedAdditions.map((addition) {
              // Extract the addition text from the key
              final parts = addition.split('-');
              if (parts.length == 2) {
                final packageKey = parts[0];
                final index = int.parse(parts[1]);
                if (packageKey == 'additions') {
                  ...
                  final additionText = items[index].toString();
                  final price = additionPrices[additionText] ?? 0.0;
