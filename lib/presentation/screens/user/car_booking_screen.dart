...
class CarBookingScreen extends StatefulWidget {
  ...
  const CarBookingScreen({super.key, required this.service});
...
class _CarBookingScreenState extends State<CarBookingScreen> {
  ...
  // Static package data for car rental
  static const Map<String, Map<String, dynamic>> _packages = {
    'carDetails': {
      'title': 'بيانات السيارة',
      'items': ['النوع: BMW 6 series', 'اللون: أسود', 'الموديل: 2022'],
    },
    'serviceDetails': {
      'title': 'تفاصيل الخدمة',
      'items': [
        'الخدمة: تأجير مع سائق',
        'مدة الحجز: حسب الطلب',
        'الاستخدام: أفراح - تصوير',
      ],
    },
    'price': {
      'title': 'السعر',
      'items': [
        'سعر الساعة: 450 جنيه',
        'سعر اليوم: 2500 جنيه',
        'السعر يشمل: الوقود + تزيين بسيط للسيارة',
      ],
    },
    'driverInfo': {
      'title': 'بيانات السائق',
      'items': ['الاسم: محمود متولي', 'السن: 25 سنة', 'الخبرة: 4 سنوات'],
    },
    'additions': {
      'title': 'الإضافات',
      'items': [
          ...
          SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                ...
                // Price Summary and Booking Button (shown after date selection)
                if (_selectedDate != null) ...[
                  _buildPriceSummary(),
                  const SizedBox(height: 16),
  ...
  Widget _buildPackagesSection() {
    return Column(
      children: [
        _buildPackageCard('carDetails'),
        const SizedBox(height: 16),
        _buildPackageCard('serviceDetails'),
        const SizedBox(height: 16),
        _buildPackageCard('price'),
        const SizedBox(height: 16),
        _buildPackageCard('driverInfo'),
        const SizedBox(height: 16),
        _buildPackageCard('additions'),
        const SizedBox(height: 16),
      ],
    );
  }

  Widget _buildPackageCard(String packageKey) {
    final package = _packages[packageKey]!;
    final items = package['items'] as List<dynamic>;
    final isAdditions = packageKey == 'additions';
      ...
      child: Column(
        ...
        children: [
          ...
          ...items.map((item) {
            ...
            final isSelected = _selectedAdditions.contains(itemStr);
              ...
              child: GestureDetector(
                onTap: isAdditions
                    ? () {
                        setState(() {
                          if (isSelected) {
                            _selectedAdditions.remove(itemStr);
                          } else {
                            _selectedAdditions.add(itemStr);
                          }
                        });
                      }
                    : null,
                child: Row(
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: isAdditions && isSelected
                            ? const Color(0xFFD4AF37)
                            : (isAdditions ? Colors.white : Colors.grey[300]),
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          ...
                          color: isAdditions && isSelected
                              ? Colors.black
                              : Colors.black87,
                          fontWeight: isAdditions && isSelected
                              ? FontWeight.w600
                              : FontWeight.normal,
                          height: 1.5,
                        ),
